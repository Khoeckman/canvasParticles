...
export const canvasParticles = function (selector, options = {}) {
  return new (class canvasParticles {
    constructor(selector = "canvas", options = {}) {
      ...
      if (!(document.querySelector(selector) instanceof Element))
        throw new ReferenceError("selector is not defined");
      ...
      this.options = {
        ...
        particles: {
          ...
          rotationSpeed: Math.max(
            0,
            (options.particles?.rotationSpeed ?? 2) / 100
          ),
        },
        gravity: {
      ...
      if (isNaN(this.options.mouse.interactionType))
        this.options.mouse.interactionType = 1;
      if (isNaN(this.options.mouse.connectDist))
        this.options.mouse.connectDist = 2 / 3;
      if (isNaN(this.options.mouse.distRatio))
        this.options.mouse.distRatio = 2 / 3;
      ...
      if (isNaN(this.options.particles.maxWork))
        this.options.particles.maxWork = Infinity;
      if (isNaN(this.options.particles.connectDist))
        this.options.particles.connectDist = 150;
      if (isNaN(this.options.particles.relSpeed))
        this.options.particles.relSpeed = 1;
      if (isNaN(this.options.particles.rotationSpeed))
        this.options.particles.rotationSpeed = 0.02;

      if (isNaN(this.options.gravity.repulsive))
        this.options.gravity.repulsive = 0;
      if (isNaN(this.options.gravity.pulling)) this.options.gravity.pulling = 0;
      if (isNaN(this.options.gravity.friction))
        this.options.gravity.friction = 0.9;
      ...
      if (this.ctx.fillStyle[0] === "#") {
        ...
        this.ctx.fillStyle =
          this.ctx.fillStyle.split(",").slice(0, -1).join(",") + ", 1)";
        this.options.particles.color = this.ctx.fillStyle;
      }
    ...
    resizeCanvas = function () {
      ...
      this.len = Math.min(
        ...
        Math.floor(
          (this.options.particles.ppm * this.width * this.height) / 1000000
        )
      );

      if (this.options.resetOnResize || typeof this.particles === "undefined")
        this.newParticles();
      else this.overflowParticles();
    };

    newParticles = function () {
      if (this.len === Infinity)
        throw new RangeError("cannot create an infinite amount of particles");
      this.particles = [];
    ...
    createParticle = function (posX, posY, dir, speed, size) {
      this.particles.push({
        ...
        speed:
          speed ||
          (0.5 + Math.random() * 0.5) * this.options.particles.relSpeed, // velocity in pixels per update
    ...
    update = function () {
      if (
        this.options.gravity.repulsive !== 0 ||
        this.options.gravity.pulling !== 0
      ) {
        for (let i = 0; i < this.len; i++) {
          for (let j = i + 1; j < this.len; j++) {
            ...
            const dist = Math.hypot(
              pointA.posX - pointB.posX,
              pointA.posY - pointB.posY
            );
            const angle = Math.atan2(
              pointB.posY - pointA.posY,
              pointB.posX - pointA.posX
            );

            if (dist < this.options.particles.connectDist / 2) {
              // apply repulsive force on all particles close together
              const grav =
                (1 / Math.max(dist, 10)) ** 1.8 *
                this.options.particles.connectDist *
                this.options.gravity.repulsive;
      ...
      for (let point of this.particles) {
        ...
        point.posX =
          (point.posX +
            point.velX +
            ((Math.sin(point.dir) * point.speed) % this.width) +
            this.width) %
          this.width;
        point.posY =
          (point.posY +
            point.velY +
            ((Math.cos(point.dir) * point.speed) % this.height) +
            this.height) %
          this.height;
        ...
        if (this.options.mouse.interactionType !== 0) {
          const distRatio =
            this.options.mouse.connectDist / Math.hypot(distX, distY);
    ...
    render = function () {
      ...
      this.ctx.fillStyle =
        this.options.particles.color + this.options.particles.opacity.hex;
      ...
      const drawAll =
        this.options.particles.connectDist >= Math.min(this.width, this.height);

      const maxWorkPerParticle =
        this.options.particles.connectDist * this.options.particles.maxWork;
            ...
            if (dist < this.options.particles.connectDist) {
              ...
              if (dist >= this.options.particles.connectDist / 2) {
                let alpha = Math.floor(
                  Math.min(this.options.particles.connectDist / dist - 1, 1) *
                    255 *
                    this.options.particles.opacity.value
                ).toString(16);
                this.ctx.strokeStyle =
                  this.options.particles.color +
                  (alpha.length === 2 ? alpha : "0" + alpha);
              } else
                this.ctx.strokeStyle =
                  this.options.particles.color +
                  this.options.particles.opacity.hex;
              ...
              if (
                (work += dist) >= maxWork ||
                (particleWork += dist) >= maxWorkPerParticle
              )
                break;
            }
          }
